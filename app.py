...
st.write("ACI 318-19 (Scale Depth x2)")
...
def draw_app():
    ...
    # Rebars MT/MB (Scale Adjusted)
    hook_vis = 12 * db_m * 2 # ขยายระยะงอเพื่อให้ดูสวยใน scale โกง
    ...
    # ET/EB (ET เขียนความยาวไว้ใต้เส้น)
    ...
    ax0.text(span_cc/2, -0.6, f"Ln = {ln:.2f} m", ha='center', weight='bold')

    ax0.set_xlim(-1, span_cc+1); ax0.set_ylim(-1.0, beam_h+1.0); ax0.set_aspect('equal'); ax0.axis('off')
    ax0.set_title(f"Longitudinal Section (Actual H={beam_h_real:.2f} m)", fontsize=14, weight='bold')

    # --- 2. Cross Sections ---
    def draw_cs(ax, title, type):
        b, h = 0.4, 0.6 # หน้าตัดใช้ scale ปกติเพื่อความคุ้นเคย
